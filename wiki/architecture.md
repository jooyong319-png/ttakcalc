...
검증 맥락: **2026-09-08**. 계산기 32종 · 사이트맵 703 URL · 테스트 222개.
...
/calendar                  세금 달력 — 법정 기한. "지금 챙길 것"만 클라이언트에서 센다
/blog  /blog/[slug]        블로그 — content/blog/*.md. 숫자는 토큰으로 데이터에서 온다
...
`content/blog/*.md`도 이 흐름에 들어온다. 본문의 `{{rates:...}}`·`{{compare:...}}` 토큰이
빌드할 때 같은 데이터·같은 계산 함수에서 값을 받는다 — **글과 계산기가 다른 답을 하는 것이
구조적으로 불가능하다.** 못 찾는 토큰이 있으면 빌드가 선다.
...
- `npm test` — 222개. 계산값 + **출처**(`sources.test.ts`) + **고유성**(`insights.test.ts`)
  + 구조화 데이터 요구사항.
- `npm run axe` — light/dark × desktop/mobile 접근성. 대비 위반은 한쪽 테마에서만,
  레이아웃 위반은 좁은 화면에서만 나서 한 조합만 보면 절반을 놓친다.
- `npm run validate:blog` — 글 길이(1,500자)·키워드(7회 이상, 밀도 3% 이하)·frontmatter.
  하한만 두면 스터핑으로 가므로 상한을 함께 본다.
